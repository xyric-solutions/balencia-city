"""
Session 35 QA import check for Relationships interior draft GLB.
...
DRAFT_GLB = os.path.join(MODULE_DIR, "interior", "drafts", "relationships-int-draft-s35.glb")
APPROVED_GLB = os.path.join(MODULE_DIR, "interior", "approved", "relationships-int.glb")
QA_JSON = os.path.join(MODULE_DIR, "interior", "drafts", "session35-qa-import.json")
...
print("=== Session 35 QA Import Check ===")
bpy.ops.wm.read_factory_settings(use_empty=True)
bpy.ops.import_scene.gltf(filepath=DRAFT_GLB)

export_pipeline = load_python_module(os.path.join(SHARED_DIR, "export-pipeline.py"), "export_pipeline_s35")
...
prop_counts = {
    "room_shell": count_objects_with_prefix(mesh_objects, ["connection_garden_room_shell"]),
    "family_bonding_dome": count_objects_with_prefix(mesh_objects, ["family_bonding_dome"]),
    "rose_energy_paths": count_objects_with_prefix(mesh_objects, ["rose_energy_path"]),
    "ai_insight_benches": count_objects_with_prefix(mesh_objects, ["ai_insight_bench"]),
    "trust_vines": count_objects_with_prefix(mesh_objects, ["trust_vines"]),
    "memory_timeline": count_objects_with_prefix(mesh_objects, ["memory_timeline"]),
    "empathy_alcoves": count_objects_with_prefix(mesh_objects, ["empathy_alcove"]),
    "low_planter_beds": count_objects_with_prefix(mesh_objects, ["low_planter_beds"]),
}
...
checks = {
    ...
    "gate5_triangle_budget_pass": 4000 <= total_tris <= 8000,
    "gate5_file_size_pass": 50 * 1024 <= file_size <= 180 * 1024,
    ...
    "gate7_focal_point_pass": prop_counts["family_bonding_dome"] >= 3,
    "gate7_props_present_pass": all(
        [
            prop_counts["room_shell"] >= 4,
            prop_counts["rose_energy_paths"] >= 2,
            prop_counts["ai_insight_benches"] >= 3,
            prop_counts["trust_vines"] >= 2,
            prop_counts["memory_timeline"] >= 2,
            prop_counts["empathy_alcoves"] >= 2,
            prop_counts["low_planter_beds"] >= 2,
        ]
    ),
}
...
report = {
    ...
    "checks": checks,
    "qa_pass": qa_pass,
    "dark_first_screenshot": "screenshots/s35-int-dark-first.png",
}
