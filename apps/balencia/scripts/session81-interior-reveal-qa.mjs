...
const reportPath = path.join(repoRoot, "output/playwright/session81-interior-reveal-static-qa.json");
...
const expectedEnergyIds = {
  leaderboard: ["hard-pipelines", "leaderboard-lightning", "ai-pulse"],
  relationships: ["warm-mist", "ai-pulse"],
  career: ["hard-pipelines", "ai-pulse"],
  recovery: ["faint-thread", "ai-pulse"],
  analytics: ["hard-pipelines", "ai-pulse"],
  nutrition: ["hard-pipelines", "ai-pulse"],
};

const requiredMarkers = [
  "leaderboard-threshold",
  "leaderboard-interior-midpoint",
  "relationships-threshold",
  "relationships-interior-midpoint",
  "career-threshold",
  "career-interior-midpoint",
  "recovery-threshold",
  "recovery-interior-midpoint",
  "analytics-threshold",
  "analytics-interior-midpoint",
  "nutrition-threshold",
  "nutrition-interior-midpoint",
];
...
for (const scene of scenes) {
  ...
  if (scene.interiorStart < 0.52 || scene.interiorStart > 0.58) {
  ...
  if (scene.interiorCameraAt < 0.66 || scene.interiorCameraAt > 0.74) {
...
if (!citySceneSource.includes("SiaInteriorRewriteLayer") || !citySceneSource.includes("sceneIndex === 3")) {
  failures.push("Session 78 SIA interior layer must remain mounted for Scene 3");
}

if (!citySceneSource.includes("DistrictInteriorRevealLayer") || !citySceneSource.includes("sceneIndex >= 4 && sceneIndex <= 14")) {
  failures.push("DistrictInteriorRevealLayer must be mounted through Scene 14");
}

if (!revealLayerSource.includes("session80-interior-reveal") || !revealLayerSource.includes("data-session81-verdict")) {
  failures.push("Session 81 reveal layer must expose DOM-visible QA markers");
}

for (const marker of requiredMarkers) {
  if (!revealLayerSource.includes(marker)) {
    failures.push(`Missing Session 81 marker: ${marker}`);
  }
}

for (const copy of [
  "Inside Competition Arena",
  "Inside Connection Gardens",
  "Inside Career Command Hub",
  "Inside Recovery Chamber",
  "Inside Data Sanctum",
  "Inside Nourishment Hall",
  "Where SIA thinks",
  "Living market",
]) {
...
if (!stylesSource.includes(".interior-reveal-cue") || !stylesSource.includes("overflow-wrap: anywhere")) {
  failures.push("Interior reveal cue CSS must keep text bounded and wrapping");
}

const noFinanceLeakageChecks = {
  knowledgebaseStatus: profiles.knowledgebase.preview.status,
  financeStatus: profiles.finance.preview.status,
  knowledgebaseContainsFinanceStatus: Object.values(profiles.knowledgebase.preview).includes(profiles.finance.preview.status),
};

if (noFinanceLeakageChecks.knowledgebaseStatus !== "Learning queue tuned") {
  failures.push("Knowledgebase status regression changed during Session 81");
}

if (noFinanceLeakageChecks.knowledgebaseContainsFinanceStatus) {
...
if (failures.length) {
  console.error(`Session 81 interior reveal QA failed. Report: ${path.relative(repoRoot, reportPath)}`);
...
console.log(`Session 81 interior reveal QA passed. Report: ${path.relative(repoRoot, reportPath)}`);
